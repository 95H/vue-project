    ...
    <section class='s-tree-edlit'>
        ...
        <ul v-show='search' class='s-tree-search-list'>
            <template v-for='(item, index) in searchArr'>
                <li :key='index' v-if='searchArr.length' v-html='handelName(item)' @mouseenter='treeItemEnter' @click='searchClick(item)'></li>
            </template>
            <!-- 搜索无数据 -->
        ...
        <section class='s-tree-con' v-show='!search'>
            ...
            <section class='s-tree-con-wrap' v-if='dataArr.length'>
                <tree-parent :data='dataArr' ref='treeParent' @change='change' @option='option' />
            </section>
            <!-- 无耗材 -->
...
<script>
import { Icon, Button } from 'meri-design'
...
export default {
    ...
    computed: {
        ...
        searchArr() {
            ...
            return this.reduceData.filter((n) => n.name && n.name.includes(this.search))
        },
    },
    methods: {
        /**
        ...
        handelName(item, level) {
            ...
            let searchName = item.name.replace(this.search, `<span style="color: #0091ff;font-size: 14px;">${this.search}</span>`)
